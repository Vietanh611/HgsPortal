...
public interface IOrgScopeService
{
    ...
    /// <summary>Kiểm tra role có thể được gán bởi caller: phải active, không phải role hệ thống, có gắn org và org đó thuộc phạm vi quản lý (anchor = User.OrganizationUnitId của caller, gồm cấp con). SUPER_ADMIN gán được mọi role không phải hệ thống.</summary>
    Task<bool> IsRoleAssignableAsync(int roleId, CancellationToken cancellationToken = default);

    /// <summary>Trả về các role caller có thể gán: active, không phải role hệ thống, thuộc org trong phạm vi quản lý (anchor = User.OrganizationUnitId của caller, gồm cấp con). SUPER_ADMIN nhận toàn bộ role active không phải hệ thống.</summary>
